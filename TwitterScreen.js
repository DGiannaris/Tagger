...
export default function TwitterScreen(props) {
  ...
  const fetchtrends = async () => {
    ...
    const postData = posts[0].trends.map((post, ind) => {
      trendsArray.push({
        data: {
          alias: post["name"],
          name: "",
          pic: "./assets/hashtag.png",
          text: ""
        }
      });
    });
console.log(trendsArray)
  ...
  const fetchtags = async val => {
    if (val !== "") {
      ...
      const posts = await twitter.get("search/tweets.json", {
        result_type: "mixed",
        q: `#${val}`
      });
      ...
      const postData = await posts.statuses.map((post, ind) => {
        handletwittArray({
          data: {
            ...
            pic: post.user["profile_image_url_https"],
            text: post.text
          }
        });
      });
  ...
  const trends = trendsArray.map((item, ind) => {
  ...
  const trendContainer = () => {
    return <View style={styles.trends}>{trends}</View>;
  };
          ...
          <View key={ind}>
            <View key={600 + ind} style={styles.twitcont}>
              ...
              <View key={700 + ind} style={styles.names}>
                <Text key={800 + ind} style={styles.alias}>
                  {item["data"].alias}
                </Text>
                <Text key={300 + ind} style={styles.item}>
                  {item["data"].name !== "" ? `@${item["data"].name}` : ""}
                </Text>
              </View>
            </View>
            <Text key={400 + ind} style={styles.listtext}>
              {item["data"].text}
            </Text>
          </View>
        </View>
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {val === "" ? (
        load ? null : (
          <Text style={styles.tagstitle}>Top Trend #Tags Worldwide</Text>
        )
      ) : null}
